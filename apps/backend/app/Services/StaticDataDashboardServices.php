    ...
    public static function getDataDashboardStatic(Request $req)
    {
        try {
           ...
           $data = [
               ...
               "attandence_islate" => \App\Models\Attandence::select("student_id")->where("attendence_late_type_id", 2)->where("attendence_late_type_id", 3)->whereDate("created_at", \Carbon\Carbon::now())->groupBy("student_id")->get()->count(),
               "attandence_notlate" => \App\Models\Attandence::select("student_id")->where("attendence_late_type_id", 1)->whereDate("created_at", \Carbon\Carbon::now())->groupBy("student_id")->get()->count(),
