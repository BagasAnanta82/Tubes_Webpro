    ...
    public static function getDataDashboardStatic(Request $req)
    {
        try {
           ...
           $data = [
               ...
               "attandence_islate" => \App\Models\Attandence::select("student_id")->where("is_late", true)->whereDate("created_at", \Carbon\Carbon::now())->groupBy("student_id")->get()->count(),
               "attandence_notlate" => \App\Models\Attandence::select("student_id")->where("is_late", false)->whereDate("created_at", \Carbon\Carbon::now())->groupBy("student_id")->get()->count(),
